...
export function Button({ title, width, loading, onClick, padding, noIcon }: ButtonProps) {
    return (
      <button
        className={`ease group relative z-30 box-border inline-flex ${
          width ? width : "w-auto"
        } ${padding} cursor-pointer items-center justify-center overflow-hidden rounded bg-orange-600 bg-gradient-to-r from-yellow-500 to-amber-500 px-8 py-3 font-bold text-white transition-all duration-300 focus:outline-none`}
        onClick={onClick}
      >
